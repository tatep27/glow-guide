...
import { ChevronDown, ChevronUp, ThumbsUp, ThumbsDown, ExternalLink, Clock } from 'lucide-react'
...
export function CareerCard({ career, experiences, onExpand }: CareerCardProps) {
  const [isExpanded, setIsExpanded] = useState(false)
  const { vote, getVote, getVoteCount } = useVoting()
  const userVote = getVote(career.id, 'career')
  const voteCounts = getVoteCount(career.id, 'career')
  ...
  const handleVote = (voteType: 'up' | 'down', e: React.MouseEvent) => {
    e.stopPropagation()
    vote(career.id, 'career', voteType)
  }
    ...
    <Card className="w-full max-w-2xl hover:shadow-2xl transition-all duration-300 border-3 border-primary/30 rounded-3xl overflow-hidden bg-gradient-to-br from-card to-card/95">
      <CardContent className="p-6">
        <div className="space-y-4">
          ...
          {/* Voting */}
          <div className="flex items-center gap-4 pt-2 border-t-2 border-border/50">
            <button
